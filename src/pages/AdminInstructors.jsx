...
import {
  ...
  FileText, Copy, ToggleLeft, ToggleRight, Users, AlertTriangle, RefreshCw,
} from 'lucide-react';
...
const EMPTY_FORM = { name: '', specialty: '', contactNumber: '', email: '', bio: '', photo: null };

export default function AdminInstructors() {
  const { instructors, members, addInstructor, updateInstructor, deleteInstructor, toggleInstructor, regeneratePortalToken, settings } = useGym();
  ...
  const handleRegen = async (inst) => {
    setRegenId(inst.id);
    try {
      await regeneratePortalToken(inst.id);
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <button
                        ...
                        className="flex items-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors"
                      >
                        <Copy size={12} /> Copy Coach Link
                      </button>
                      <button
                        onClick={() => handleRegen(inst)}
                        disabled={regenId === inst.id}
                        title="Generate a new link (old one stops working)"
                        className="flex items-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-slate-400 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-50"
                      >
                        <RefreshCw size={12} className={regenId === inst.id ? 'animate-spin' : ''} /> New Link
                      </button>
                    </div>
                  </div>
          ...
          <div className="bg-slate-800 rounded-2xl w-full max-w-sm border border-slate-700 shadow-2xl p-6 space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white py-3 rounded-xl font-bold transition-colors flex items-center justify-center"
              >
                {deleting

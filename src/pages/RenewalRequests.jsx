        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map((req) => {
              ...
              return (
                <div key={req.id} className="bg-slate-800 rounded-2xl border border-slate-700/50 overflow-hidden">
                  {/* Card header */}
                  <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-slate-700/50">
                    <div>
                      <p className="text-white font-bold text-lg leading-tight">{req.member_name}</p>
                      <p className="text-slate-400 text-sm mt-0.5">{req.contact_number}</p>
                    </div>
                    <span className={`shrink-0 text-xs font-semibold px-3 py-1 rounded-full border ${cfg.bg} ${cfg.color}`}>
                      {cfg.label}
                    </span>
                  </div>
          ...
          <div className="bg-slate-800 rounded-2xl w-full max-w-sm border border-slate-700 shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              <textarea
                ...
                placeholder="Reason for rejection (optional — member won't see this)"
